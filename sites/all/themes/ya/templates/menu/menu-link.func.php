 ...
 */
function ya_menu_link(array $variables) {
    ...
    $title_link = l($title, $href, $options);
    ...
    if ($element['#below']) {
        ...
        elseif ((!empty($element['#original_link']['depth']))) {
            ...
            $title_link .= ' <a href="#dropdown-' . $mlid . '-' . $element['#original_link']['depth'] . '" class="dropdown-link collapsed" data-toggle="collapse"><span class="glyphicon glyphicon-menu-down dropdown-caret"></span></a>';
            $title_link = '<div class="title-link">' . $title_link . '</div>';
    ...
    return '<li' . drupal_attributes($attributes) . '>' . $title_link . $sub_menu . "</li>\n";
}
 ...
 */
function ya_menu_link__user_menu(array $variables)
{
    $element = $variables['element'];
    $mlid = $element['#original_link']['mlid'];
    ...
    if (in_array($mlid, [14, 12880])) {
        $options['attributes']['class'][] = 'btn-s1';
        $title =  '<img src="/sites/all/themes/ya/images/icons/login.png" alt="sign in" />';
  ...
  if ($mlid == 15373) {
    $options['attributes']['class'][] = 'btn-s3';
    $options['attributes']['class'][] = 'popup-trigger';
    $options['attributes']['id'][] = 'cart';
    $title =  '<img src="/sites/all/themes/ya/images/icons/cart.png" alt="cart" />';
    $title .=  '<div class="popup popup-top-right"><div class="popup-content">' . t('Cart'). '</div></div>';
  }
 ...
 * @ingroup theme_functions
 */
function ya_menu_link__menu_main_d(array $variables) {
    $element = $variables['element'];
    $sub_menu = '';
        ...
        if ($source_term_wr = entity_metadata_wrapper('taxonomy_term', $source_tid)) {
            ...
            if ($depth == 2) {
                ...
                if ($list) {
                    ...
                    $list_html = ya_pack_list_to_html($list, empty($chevron_color) ? [] : ['color' => $chevron_color]);
                    ...
                    if ($element['#below']) {
                        foreach ($element['#below'] as $key => $item) {
                            if (is_numeric($key) && $banner_html = ya_get_banner_html($element['#below'][$key])) {
...
function ya_get_menu_terms($menu_list)
{
    $term_tids = $terms = [];
...
function ya_get_menu_entities($field_entity, $field_entity_field = null, $tids = [])
{
    $items = [];
...
function ya_pack_list_to_html($items, $options)
{
    $list = '';
    if ($items) {
...
function ya_get_banner_html($element)
{
    $html = '';
