...
//                $color_f = 'rgba(' . hexdec($color_d[0]) . ', ' . hexdec($color_d[1]) . ', ' . hexdec($color_d[2]) . ', ' . '0.2)';
                ...
                if ($h_groups) $hobjects = '<h4 class="bg-lighter">Группы</h4>' . implode(' ', $h_groups);
                if ($hovids) $hobjects .= '<h4 class="bg-lighter">Виды</h4>' . $hovids;
                ...
                $table .= '<tr>'
                    . ($count_f ? '' : '<td rowspan_f="" class="text90 bg-main"><div>' . $function . '</div></td>')
                    . ($count_c ? '' : '<td rowspan_c="" class="bg-light">' . $cultures . '</td>')
                    . ($count_h ? '' : '<td rowspan_h="" class="bg-light">' . $hobjects . '</td>')
                    . ($count_rp ? '' : '<td rowspan_rp="" align="center" class="bg-light rate' . ($row_counter ? '' : ' active') .'">' . $rate_p . '</td>')
                    . ($count_rm ? '' : '<td rowspan_rm="" class="bg-light" align="center">' . $rate_m . '</td>')
                    . ($count_d ? '' : '<td rowspan_d="" class="bg-light">' . $descr . '</td>')
                    . (($count_p || ($option == AGRO_CATEGORY_FERT_TID)) ? '' : '<td rowspan_p="" class="bg-light" align="center">' . trim($period . ' ' . $multi) . '</td>')
                    . (user_access('access product_reglaments actions') ? '<td align="center" class="actions bg-light">' . implode(',<br />', $links) . '</td>' : '')
                    . '</tr>';
                $count_f++;
                $row_counter++;
            }
    ...
    <table class="calculation">
        <tr><th colspan=2 class="calc-title">Калькулятор стоимости обработки<br /><span>выберите ячейку с нормой расхода в таблице выше и задайте <?php print $volume; ?></span></th></tr>
        <tr>
